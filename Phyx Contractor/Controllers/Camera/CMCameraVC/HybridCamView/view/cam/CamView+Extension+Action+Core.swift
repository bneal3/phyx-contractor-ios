...
extension CamView{
     ...
     */
    func takePhoto() {
        guard let connection = photoOutput.connection(with: .video) else {onVideoCaptureComplete(nil,CaptureError.noVideoConnection);return}
        guard let device:AVCaptureDevice = deviceInput?.device else {onVideoCaptureComplete(nil,CaptureError.noInputDevice);return}
        if device.isSmoothAutoFocusSupported {
            do {
                try device.lockForConfiguration()
        ...
        // Keeps front facing pictures as they look on screen
        if device.position == AVCaptureDevice.Position.front {
            connection.isVideoMirrored = true
        }
